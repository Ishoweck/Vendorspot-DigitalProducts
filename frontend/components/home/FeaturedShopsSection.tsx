      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
          {featuredShops.map((shopName, index) => (
            <div
              key={index}
              className={`bg-white rounded-lg hover:shadow-lg transition-shadow duration-200 w-full ${index >= 2 ? "hidden sm:flex" : "flex"} items-center justify-center text-center`}
              style={{
                borderRadius: "8px",
                aspectRatio: "227/203",
                maxWidth: "227px",
